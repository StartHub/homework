...
public interface JedisCommands {
    ...
    /**
     * 机制同{expire method}一样，只是时间单位改为毫秒。
     *
     * @param key
     * @param milliseconds
     * @return 返回值同 {expire method}一样。
     */
    Long pexpire(String key, long milliseconds);

    /**
     * 与{expire method}不一样，expireAt设置的时间不是能存活多久，而是固定的UNIX时间（从1970年开始算起），单位为秒。
     *
     * @param key
     * @param unixTime
     * @return
